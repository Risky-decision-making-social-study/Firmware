...
from monkey_list import MONKEYS, getMonkeyDBVariablesSampling2
...
(
    ...
    CONTROLLER_ID,
    DISTRIBUTOR_ID,
) = getMonkeyDBVariablesSampling2(MONKEYS["DUMMY"])
CONDITION = "quantity"
SESSION_TYPE = "sampling_session_2"
...
RISK_LEVER_COUNT = sum(RISK_LEVER)
...
PSEUDO_RANDOM_RISK_OUTCOME = [
    LOS_REWARD_OUTCOME if i else WIN_REWARD_OUTCOME for i in util.apparatus_pseudo_random_generator(RISK_LEVER_COUNT)]

# sampling session type determination
if RISK_LEVER[0]:
    SESSION_TYPE += "-risk_first"
else:
    SESSION_TYPE += "-safe_first"
...
APPARATUS_COLOR = COLOR_RISK if RISK_LEVER[0] else COLOR_SAFE
...
for trial_id in range(0, 16):
    ...
    APPARATUS_COLOR = COLOR_RISK if RISK_LEVER[trial_id] else COLOR_SAFE
    ...
    # --- Modifiy PSEUDO_RANDOM_RISK_OUTCOME
    # TODO
    risk_session_log_pull = session_log_pull if RISK_FIRST else session_log_pull[8:]
    PSEUDO_RANDOM_RISK_OUTCOME = util.random_list_modifier(
        risk_session_log_pull, PSEUDO_RANDOM_RISK_OUTCOME)

    # --- Move to prepare next trial
    time.sleep(DEPLOY_DELAY)
    if trial_id < 15:
        if RISK_LEVER[trial_id + 1]:
            # risk
            apparatus.move_to(apparatus.CAROUSEL_2, util.pop_deploy(
                RISK_BAIT_PATTERN, PSEUDO_RANDOM_RISK_OUTCOME[(trial_id + 1) % RISK_LEVER_COUNT], write=False),
                monkey=DISTRIBUTOR_MACHINE, blocking=False)
        else:
            # safe
            apparatus.move_to(apparatus.CAROUSEL_1, util.pop_deploy(
